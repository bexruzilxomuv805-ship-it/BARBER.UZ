...
export default function Register() {
  ...
  // Set once the site's own Register form succeeds — swaps the form out for
  // the mandatory TelegramLinkGate step instead of navigating away right
  // away, so an account created this way never ends up without a telegramId
  // (TelegramLoginButton below already produces a linked account by
  // construction, so it skips straight to handleTelegramSuccess).
  ...
  const handleSubmit = async (e) => {
    ...
    if (registerUser.fulfilled.match(result)) {
      setRegisteredUser(result.payload)
    }
  }
  ...
  const handleLinked = () => {
    dispatch(showToast({ type: 'success', text: t('register.welcomeToast', { name: registeredUser.ism }) }))
    navigate('/')
  }
    ...
    <div className="relative flex min-h-[calc(100vh-72px)] items-center justify-center overflow-hidden py-16">
      ...
      >
        <div className="card p-8 shadow-gold">
          {registeredUser ? (
            <TelegramLinkGate userId={registeredUser.id} onLinked={handleLinked} />
          ) : (
            <>
              <div className="flex flex-col items-center text-center mb-6">
                ...
                <h1 className="mt-4 font-display text-2xl font-bold text-strong">{t('register.title')}</h1>
